...
Relvar.prototype.compare = function(a, b) {
	return relvarUtils.compareKey(this.uniqueKey, a, b);
};
	...
	if(replacedRows.length > 0) {
		process.nextTick(function() {
			replacedRows.forEach(function(row, index) {
				// @TODO save operation if new row has same values for uniqueKey
				this.index.remove(row);
				this.index.insert(newRows[index]);
			}.bind(this));
			this.emit("update", replacedRows, newRows);
			this.emit("change", "update", replacedRows, newRows);
			if(cb) {cb();}
		}.bind(this));
	}
	return this;
...
Relvar.prototype.remove = function(keys, cb) {
	...
	if(removedRows.length > 0) {
		process.nextTick(function() {
			...
			this.emit("change", "remove", removedRows);
			if(cb) {cb();}
		}.bind(this));
	}
	return this;
