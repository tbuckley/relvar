...
function join(relvarLeft, relvarRight, uniqueKey) {
	var r, overlapKey, leftIndex, rightIndex, cmpFunc;
	...
	r = new Relvar(relvarLeft.spec, relvarLeft.uniqueKey);
	relvarLeft.on("insert", function(rows) {
		// For each row added, add any joins with rows in relvarRight
		var insert;
		insert = [];
		rows.forEach(function(row) {
			addToIndex(overlapKey, leftIndex, row);
			forEachJoin(overlapKey, leftIndex, row, function(joinedRow) {
				insert.push(joinedRow);
			});
		});
		r.insert(insert);
	});
	relvarLeft.on("update", function(keys, rows) {
	...
	function addToIndex(indexKey, index, row) {
		var key, data;
		key = utils.project(row, indexKey);
	...
	function removeFromIndex(indexKey, uniqueKey, index, row) {
		var key, data, i;
		key = utils.project(row, indexKey);
		data = index.find({key: key});
		for(i = 0; i < data.rows.length; i++) {
			if(utils.compareKey(uniqueKey, data.rows[i], row) === 0) {
				data.rows.splice(i, 1);
				return;
			}
		}
	}
	function forEachJoin(indexKey, index, row, fn) {
		key = utils.project(row, indexKey);
		data = index.find({key: key});
	...
	relvarLeft.on("remove", function(rows) {
		// For each row removed, remove any joins with rows in relvarRight
		var remove = [];
		rows.forEach(function(row) {
			removeFromIndex(overlapKey, relvarLeft.uniqueKey, leftIndex, row);
			forEachJoin(overlapKey, rightIndex, row, function(joinedRow) {
